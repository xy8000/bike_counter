...
export function SharePie({
  slices,
  className,
}: Readonly<{ slices: ShareSlice[]; className?: string }>) {
  const data = slices.filter((slice) => slice.total > 0)
  const config: ChartConfig = { total: { label: 'Bikes' } }

  // A per-channel/per-station pie with more than MAX_DATA_STREAMS slices is
  // unreadable; show an info note instead of the donut + legend.
  let body: ReactNode
  if (data.length === 0) {
    body = <ChartEmptyState message="No traffic for this period." className="py-16" />
  } else if (data.length > MAX_DATA_STREAMS) {
    body = <ChartLimitNotice className="py-16" />
  } else {
    body = (
      <>
        <ChartContainer
