...
- [x] Domain: `DataSource.first_measurement_at` / `last_measurement_at` (+ `new()`) and `DataSourceRepository::update_measurement_bounds` (default no-op; Postgres merges NULL-safe with LEAST/GREATEST)
...
- [x] Migration V20: covering btree index `measurements_timestamp_idx` on `measurements (timestamp) INCLUDE (channel_id, resolution_seconds, value)`, turning the global-summary / sidebar / summary `bikes_last_day` windowed `SUM` into an index-only range scan instead of a sequential scan over the whole measurement history (the previous indexes all led with `channel_id`, so the time-range predicate was not indexable)
...
Bike-Trends: exclude new stations from trends (plan 76)

- [x] Backend: generic full-window coverage predicate `has_full_coverage` (reuses the existing `covers` in `station_analytics/resolution.rs`) + `ChannelCoverage` / `resolution_coverage_by_channel` on the measurement repository port (Postgres + in-memory mocks)
- [x] Backend: `metric_windows` / `period_data` filter out stations that lack full coverage of the current **and** previous window when `exclude_new_stations` is on (summary like-for-like); the single-station detail page keeps the totals and reports `is_new` instead
- [x] Backend: `global_summary` restricts `bikes_last_day_total` to fully-covered stations; `StationAnalyticsServicePort`/service gain the flag
- [x] BFF: `exclude_new_stations` query param on the detail overview/graphs, summary overview/graphs, summary monthly and global-summary endpoints; `is_new` on `MetricDto`/`PeriodGraphsDto`; OpenAPI updated
- [x] Backend: `stations_summary_monthly` drops stations lacking full current + previous calendar-year coverage when `exclude_new_stations` is on (the `year`-timeframe rule), so the summary's monthly bar chart responds to the slider
- [x] Frontend: `features/settings` (localStorage-backed `TrendSettingsProvider`/`useTrendSettings` + `SettingsDialog` opened from a page-level "Calculation settings" button on the detail and summary pages); the global summary, summary cards and detail cards append the flag; `MetricCard` shows a neutral "New" and the detail graphs show a notice for `is_new`
- [x] Bugfix: the "whole window" check treated the still-running current week/year window (ending at `now`) as if it had ended, requiring a measurement within one resolution interval of `now`; once the data lagged `now`, every established station was dropped and all summary graphs came back empty. `covers_whole_window`/`has_full_coverage` now treat a running window (`to >= now`) as never gating — its data may not have arrived yet (incomplete week, import lag, stale seed) — so only the completed (previous) window decides; finished windows still require data through their end. Regression tests `established_stations_with_a_stale_last_measurement_are_not_dropped` and `stations_summary_graphs_keep_a_station_with_no_current_week_data`
- [x] Tests: unit (metrics/graphs/global full-coverage filtering + `is_new`, resolution `has_full_coverage`/`covers_whole_window`, stale-latest-measurement regression), REST (`is_new` + flag acceptance), Playwright `settings.spec.ts` (toggle + persistence)
- [x] Gates green: `make check`, `make test` (465), `make test-rest` (99), `make coverage` (overall 87.09%, core 95.57%), `make test-playwright` (31)

Station flags (active / selected / inactive), banner + popup, full-width share, Playwright timeouts (plan 78)

- [x] Backend: `V17__add_counting_station_status.sql` adds `counting_stations.status` (`active` default) + CHECK; `CountingStation.status` (`StationStatus` value object, `Default = Active`); Postgres repo reads/writes it
- [x] Backend: `sync_counting_stations` reactivates stations still in the provider output and marks stations of the data source that are missing from it as `inactive`; unit tests for both paths
- [x] Backend: stations without current data are marked `inactive` too — `latest_by_channel` (`SELECT DISTINCT ON (channel_id)` in the Postgres repo) feeds `mark_stale_stations_inactive` (48 h window) which runs after every `import()` / `update_data_source()`; marking is one-way (reactivation only via the provider sync + fresh data); unit tests incl. `marks_a_station_inactive_when_it_has_no_current_data`
- [x] BFF: `StationMapDto.status` (`active`/`inactive`) from the persisted status; the BFF stays stateless — `selected` is derived by the frontend from the URL `station` param (no new query param); REST tests for active + inactive markers
- [x] Frontend: three Vite flag SVGs `station-flag.svg` / `station-flag-selected.svg` / `station-flag-inactive.svg` (old `map-flag-counting-station.svg` deleted); `stationMarkerImage` gains a `state` option + `station-marker--selected`/`--inactive` classes; `StationMap.status` type; `MapView` resolves selected from `selectedStationId` and `DetailMap` renders the active flag; an inactive station stays inactive even when selected (inactive beats selected)
- [x] Overview banner: name + description + channel-count badge (description/channel moved out of the body; no separate icon per review); map popup laid out with the icon top-left + name beside it, the channel count as an overview-style badge and the description wrapping on a line below (pinned `maxWidth` + `break-words` so long text/buttons never overflow); `DetailMap` preview uses the active (not selected) flag
- [x] "Share by channel" (detail) and "Share by station" (summary) charts moved to their own full-width rows in the nerd-stats sections
- [x] Playwright: `globalTimeout`, `use.actionTimeout` + `use.navigationTimeout` added (generous, so healthy runs are unaffected)
- [x] e2e: `flags.spec.ts` (marker click → selected flag + URL, void click clears, search + Find on map → selected flag, inactive station → inactive flag); `map.spec.ts` popup + overview-banner assertions; the e2e script marks `Gartenstraße` inactive after startup (the seed lags V17, so the status column only exists after refinery applies it)
- [x] Gates green: `make check`, `make test` (474), `make test-rest` (100), `make coverage` (overall 87.25%, core 95.45%), `make test-playwright` (41)
